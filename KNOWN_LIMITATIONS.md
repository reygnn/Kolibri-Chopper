...
`sigil.json` (that file is the user's config: favorites, hidden apps, custom
...
state, and keeping them out of `sigil.json` avoids write churn and keeps the
...
file (not `sigil.json`) and load it on start.
